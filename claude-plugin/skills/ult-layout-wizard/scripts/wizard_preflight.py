#!/usr/bin/env python3
"""wizard_preflight.py - fail-fast dependency gate for ult-layout-wizard (D24 §18.10
...
1. `ult-repo-layout` is present at all (`.github/skills/ult-repo-layout/SKILL.md`
   ...
   `install.ps1 -Only "ult-layout-wizard"` - a fully valid, unblocked install today,
...
def _check_present(repo_root: Path) -> None:
    ...
    if not skill_md.exists():
        raise PreflightError(
            "ult-layout-wizard requires ult-repo-layout, but "
            f"{skill_md} was not found.\n"
            "If this was installed with `install.ps1 -Only \"ult-layout-wizard\"` "
            ...
            '  install.ps1 -Only "ult-repo-layout,ult-layout-wizard"'
        )
