...
- anything already sitting in `starter_kit/project_guidelines/` (except
  `.pointer.md`)
- the resolved `how_dimension.how_l2.path` directory, if set in
  `context-config.yaml` — glob its contents. This is where
  `ult-autoscaffold-content` writes `CODING-STANDARDS.md`,
  `TESTING-GUIDELINES.md`, per-module `CONTEXT.md` files, and
  `interfaces/*.md` when a project has none of these yet; once they exist,
  this step's job is to fold them in like any other source, not to
  regenerate them.
...
can't see. For binary formats, read them with whatever's available in this project — a
PDF/DOCX-reading skill if one is installed, or a lightweight library you install for the occasion
(e.g. `pdfplumber`/`python-docx`) — and extract the plain text before folding it into the
compiled file.
...
If the source file's own frontmatter carries `generated_by:
ult-autoscaffold-content` and `status: draft`, note that alongside its
`SOURCE` line — a scaffold-generated draft rule is real input, but the
final compiled file's `Sources:` footer (step 6) should say so, same as it
already states each source's modified date.
...
>   - <source file> (modified <date>, scaffold-generated draft — see its own frontmatter)
