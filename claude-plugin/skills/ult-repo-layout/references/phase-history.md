...
The six illustrative slots below exist to prove the registry/marker
mechanism generalizes past a single owner (`context_packages`) to any number
of independent producer/consumer skills — see Phase 2's exit criterion for
the actual proof.
...
...plus the **`project_guidelines` starter-kit drop-zone** (D21 §16.6, Phase
3d). This is **not** a `project_layout` path-slot (no marker, no resolution
algorithm) — it's a regenerated `.pointer.md` scaffold file; see
`references/starter-kit-dropzones.md`.
...
- **Phase 3b** — a `plans_output`/`brainstorm_output` marker pointing
  ...
  retrofitted files (2 producers + 3 consumers, §16.4) resolve to the
...
- **Phase 3d** — `init` on an empty repo produces a fully-wired
  ...
  `project_name`/`description`) plus the `project_guidelines` starter-kit
  `.pointer.md` file, with no `starter_kits/` copy — the directories
...
- **Phase 2** — each of the five new slots (`compiled_guidelines`,
  ...
  covering a `kind: file` slot and the partial-install gate).
- **Phase 3e** — `init` on an empty repo with `layout.workspace_root` set
  ...
  `references/library-level-registry.md`), a registry-consistency check (#12)
