...
description: Compile scattered guideline sources into one scope-aware COMPILED-GUIDELINES.md for spw skills and ult-context-generate's Constraints layer. Do NOT use to enforce rules at runtime.
namespace: spw
version: 0.2.0
origin: ground-up
...
produces a single, predictable, human-readable file that every other `spw` skill can cite before
it reads, writes, modifies, reviews, or judges project code.

**Run this:**
- Once, during project setup (`/spw-compile-guidelines`)
...
> Generated by /spw-compile-guidelines on <YYYY-MM-DD>
> Sources:
>   - <source file> (modified <date>)
>   - <source file> (modified <date>)
> Re-run /spw-compile-guidelines after changing any source above — this file does not auto-update.

## How spw skills should use this file
...
- `## Recent Observations (pending compile)` is also append-only between re-runs, but for a
  ...
  compile runs. Triage it on the next `/spw-compile-guidelines` run (see "On re-run" below);
...
When `/spw-compile-guidelines` is invoked and the resolved `compiled_guidelines`
...
- It does not change how `/spw-write-plan`, `/spw-execute-plan`, or any other `spw` skill
  executes its own work. It only gives them one more file to read first, when one exists. See
  `CONSUMING-COMPILED-GUIDELINES.md` (colocated in this skill's directory) for the exact protocol
  every code-facing `spw` skill follows to consume the file this skill produces.
