"""Regression suite for atomic_write.py (2026-09-23 torn-read fix). Mirrors
ult-cep-wizard's test_wizard_atomic_write.py 1:1, since this module is a
deliberate duplicate of wizard_atomic_write.py (see atomic_write.py's module
docstring for why it isn't a cross-import). Stdlib unittest only. Run with:
...
class TestWriteTextAtomic(unittest.TestCase):
    ...
    def test_missing_parent_directory_raises_and_writes_nothing(self):
        ...
        with self.assertRaises(aw.AtomicWriteError):
            aw.write_text_atomic(target, "content\n")
    ...
    def test_failure_during_replace_leaves_target_untouched_and_cleans_up_temp(self):
        ...
        with mock.patch("os.replace", side_effect=OSError("simulated failure")):
            with self.assertRaises(aw.AtomicWriteError):
                aw.write_text_atomic(target, "attempted overwrite\n")
    ...
    def test_transient_windows_permission_error_on_replace_is_retried(self):
        # Same real-world Windows AV/indexer race wizard_atomic_write.py's
        # equivalent test documents - a single transient PermissionError on
        # os.replace must not fail the whole write.
    ...
    def test_replace_with_retry_misconfigured_to_zero_attempts_raises_assertion_not_typeerror(self):
        ...
        with mock.patch.object(aw, "_REPLACE_RETRY_ATTEMPTS", 0):
            with self.assertRaises(AssertionError):
                aw._replace_with_retry(self.root / "src.tmp", target)
