...
- **Already has a marker (anywhere)** → report "`<slot>` — unaffected,
  already has a marker at `<marker path>`" (§16.11) and move on. A
...
- **No marker, content exists at BOTH the pre-D21 default AND the
  `{workspace_root}/<leaf>` default** → flag a conflict (§16.11)
...
The same report additionally covers the `project_guidelines` starter-kit
drop-zone (see `references/starter-kit-dropzones.md`): current path
(`starter_kit/project_guidelines/`) vs.
`{workspace_root}/inputs/starter-kit/project_guidelines/`, with the same
`mv`/`git mv` suggestion (option (b) shape) if the human wants to re-root its
content. The drop-zone has no marker mechanism, so option (a) doesn't apply
to it — after a human-run `mv`, the next `init`/`reconcile` simply
regenerates `.pointer.md` at the new location (it already follows "current
location," per that section).
...
content, and `workspace_root` changes *defaults only* (§16.2).
