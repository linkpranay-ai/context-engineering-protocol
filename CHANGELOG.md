...
- **Claude Code plugin package**: `claude-plugin/` packages the 4 real skills
  (`compiling-project-guidelines`, `ult-codegraph`, `ult-context-generate`, `ult-repo-layout`) as an
  installable Claude Code plugin, generated by `catalog/export_claude_plugin.py` from
  `.github/skills/` (the single source of truth) so the package can't silently drift from the real
  skills. `demo-consume-context` is deliberately excluded — its own frontmatter marks it "do NOT use
  for real feature work". Listing copy foregrounds `ult-context-generate` as the skill to try first,
  the rest presented as supporting tools. Verified with a real local one-click install
