...
| [cep-retrofit-mattpocock-skills](cep-retrofit-mattpocock-skills/CASE-STUDY.md) | mattpocock/skills (MIT) + Textualize/textual (MIT) | No | First test of the `ult-cep-retrofit` metaskill itself: a full-library retrofit pass across a real, unrelated 71-unit skill library (0 missed skill units, 0 incorrectly-claimed non-skill scaffolding), plus a deep with/without-CEP comparison of one retrofitted skill (`to-spec`)'s generated output, plus a trip-wire (institutional-memory) rung on top. Finds a real Mode 2 rendering-mechanism error and a fabricated test-file path Mode 1/3 avoid, and a genuine Windows path-length (`MAX_PATH`) environment bug in the metaskill's own tooling, found and fixed along the way. Opens this directory's `Trip-wire` and `Metaskill-retrofit origin` coverage. |
| [cep-retrofit-superpowers](cep-retrofit-superpowers/CASE-STUDY.md) | obra/superpowers (MIT) + open5gs/open5gs (AGPL-3.0) | No | Second test of the `ult-cep-retrofit` metaskill: a full-library retrofit pass across a real, unrelated 62-unit skill library (0 misclassifications, pristine clone rather than this repo's own already-adapted copies of the same skills), plus a deep with/without-CEP comparison of one retrofitted skill (`writing-plans`)'s generated output, plus a trip-wire (institutional-memory) rung on top. Finds real Mode 2 defects (duplicate AVP declarations, a fabricated test target) Mode 1 avoids, and a genuinely humbling trip-wire finding: a `tier: revise` hit accepted without checking its own `required_evidence` field produces a confidently wrong value. |
...
| cep-retrofit-mattpocock-skills | ✅ (reused a pre-existing approved package, substituted, disclosed) | ✅ | ✅ (0 conflicts, 1 gap, inherited from the source package) | ➖ | ✅ (3-entry fixture ledger, real `query`, 1 of 3 hits materially changed the Testing Decisions section) | Yes — metaskill-retrofitted `to-spec`, human-overridden `recommend()` signal (a disclosed demonstration that the signal is advisory, not a decision) | Generative |
...
composition remains unimplemented everywhere. `cep-retrofit-mattpocock-skills` and
`cep-retrofit-superpowers`, run together against two real, unrelated skill libraries, are the only
two cases to exercise **trip-wire** and **metaskill-retrofit origin** so far. A separate, one-off
GitHub Copilot cross-runtime re-run of the `mattpocock/skills` experiment (checking whether
`ult-cep-retrofit`'s deterministic, script-driven mechanism behaves identically regardless of which
agent operates it) surfaced a real `cep_retrofit.py` CLI bug (`CHANGELOG.md`'s `[Unreleased]` →
Fixed) but is itself intentionally not kept in this directory — it carries known, uncorrected
errata from the Copilot run and duplicates rather than adds to the case above; see
`cep-retrofit-mattpocock-skills-copilot/OBSOLETE.md` (gitignored, local-only) for the full
disposition. No third metaskill-retrofit case study is currently planned. Derived-package
composition still awaits its own first case.
