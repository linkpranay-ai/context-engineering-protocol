...
- **No installer script exists.** Several `SKILL.md` files reference `install.ps1`/`install.sh
  -InitProject` as how a consuming project gets set up, but no such script exists anywhere in this
  repo yet — adoption today means manually copying `.github/skills/`, `.github/prompts/`, and
  `AGENTS.md`/`.cursor/rules/` into the target project. Confirmed painful but workable during the
  Phase 9 dogfood run; building the installer is a candidate for a later pass.
